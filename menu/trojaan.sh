...
read -p "   Traffic Limit (GB): " traffic_limit # New line to ask for traffic limit
...
# Insert user details into the Xray config
sed -i '/#trojan$/a\#trx '"$user $exp $harini $uuid $traffic_limit"'\
},{"id": "'""$uuid""'","password": "'""$user""'","email": "'""$user""'","traffic_limit": "'"$traffic_limit"'"}' /usr/local/etc/xray/config.json

# Save user details with traffic limit
echo -e "### $user $exp $harini $uuid $traffic_limit" >> /usr/local/etc/xray/akunxtr.conf
...
echo -e "Expired : $exp"
echo -e "Traffic Limit: ${traffic_limit} GB" # Display the traffic limit
echo -e "Script By $creditt"
}
function menuCheckTraffic () {
    clear
    ...
    # Extract user details from akunxtr.conf
    ...
    total_gb=$(echo "scale=2; $total_bytes/1073741824" | bc) # Convert to GB

    # Calculate remaining traffic
    remaining_gb=$(echo "scale=2; $traffic_limit - $total_gb" | bc)
    ...
    echo -e "Traffic Used    : ${total_gb} GB"
    echo -e "Traffic Left    : ${remaining_gb} GB" # Show remaining traffic
    echo -e "\e[$line═════════════════════════════════\e[m"
    echo -e "Script By $creditt"
}
...
echo -e "   \e[$number (•7)\e[m \e[$below Check Remaining Traffic\e[m" # New option
echo -e ""
echo -e "   \e[$number    >> Total :\e[m \e[$below ${total} Client\e[m"
echo -e "   \e[$line════════════════════════════════════════\e[m"
echo -e "   \e[$back_text            \e[30m═[\e[$box TROJAN GO\e[30m ]═             \e[m"
echo -e "   \e[$line════════════════════════════════════════\e[m"
echo -e "   \e[$number (•8)\e[m \e[$below Create Trojan Go Account\e[m"
echo -e "   \e[$number (•9)\e[m \e[$below Trial Trojan Go Account\e[m"
echo -e "   \e[$number (10)\e[m \e[$below Deleting Trojan Go Account\e[m"
echo -e "   \e[$number (11)\e[m \e[$below Renew Trojan Go Account\e[m"
echo -e "   \e[$number (12)\e[m \e[$below Show Config Trojan Go Account\e[m"
echo -e "   \e[$number (13)\e[m \e[$below Check User Login Trojan Go\e[m"
echo -e ""
...
echo -e "   \e[$line════════════════════════════════════════\e[m"
echo -e "\e[$line"
read -rp "      Please Input Number  [1-13 or x] :  "  num
echo -e ""
if [[ "$num" = "1" ]]; then
menu1
elif [[ "$num" = "2" ]]; then
...
menuCheckTraffic # New function call
elif [[ "$num" = "8" ]]; then
menu7
elif [[ "$num" = "9" ]]; then
menu8
